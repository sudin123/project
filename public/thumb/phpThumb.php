...
function SendSaveAsFileHeaderIfNeeded(&$phpThumb)
{
    ...
    if (@$downloadfilename) {
        $phpThumb->DebugMessage('SendSaveAsFileHeaderIfNeeded($phpThumb) sending header: Content-Disposition: ' . (@$_GET['down'] ? 'attachment' : 'inline') . '; filename="' . $downloadfilename . '"', __FILE__, __LINE__);
